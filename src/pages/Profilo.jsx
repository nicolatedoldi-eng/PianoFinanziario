...
export default function Profilo() {
  ...
  const createdAt = new Date(profile.created_at).toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' })
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 shadow-sm">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <div className="text-sm mt-1" style={{ color: portfolio.color }}>
              Rendimento atteso: <strong>{portfolio.expectedReturn}%</strong> annuo
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 pt-4 grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
          ...
          <div>
            ...
            <div className="font-medium text-gray-900">
              {profile.last_rebalance_at
                ? new Date(profile.last_rebalance_at).toLocaleDateString('it-IT')
                : 'Non ancora fatto'}
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          ...
          {!editing && (
            <button onClick={() => setEditing(true)} className="text-sm px-4 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
              Modifica
            </button>
          )}
        </div>
          ...
          <div className="space-y-6">
            <div>
              <div className="flex justify-between text-sm mb-1.5">
                ...
                <span className="font-semibold">\u20ac{editParams.initialCapital.toLocaleString('it-IT')}</span>
              </div>
              <input type="range" min="0" max="100000" step="500" value={editParams.initialCapital}
                onChange={e => setEditParams(prev => ({ ...prev, initialCapital: Number(e.target.value) }))}
                className="w-full accent-[#534AB7]" />
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1.5">
                <span className="text-gray-600">PAC mensile</span>
                <span className="font-semibold">\u20ac{editParams.monthlyPayment.toLocaleString('it-IT')}</span>
              </div>
              <input type="range" min="50" max="2000" step="50" value={editParams.monthlyPayment}
                onChange={e => setEditParams(prev => ({ ...prev, monthlyPayment: Number(e.target.value) }))}
                className="w-full accent-[#534AB7]" />
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1.5">
                <span className="text-gray-600">Orizzonte</span>
                <span className="font-semibold">{editParams.horizon} anni</span>
              </div>
              <input type="range" min="1" max="35" step="1" value={editParams.horizon}
                onChange={e => setEditParams(prev => ({ ...prev, horizon: Number(e.target.value) }))}
                className="w-full accent-[#534AB7]" />
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1.5">
                <span className="text-gray-600">Crescita PAC annua</span>
                <span className="font-semibold">{editParams.annualGrowth}%</span>
              </div>
              <input type="range" min="0" max="10" step="0.5" value={editParams.annualGrowth}
                onChange={e => setEditParams(prev => ({ ...prev, annualGrowth: Number(e.target.value) }))}
                className="w-full accent-[#534AB7]" />
            </div>
            <div className="flex gap-3">
              <button onClick={handleSaveParams} disabled={saving}
                className="flex-1 py-3 rounded-xl text-white font-semibold transition-opacity hover:opacity-90 disabled:opacity-60"
